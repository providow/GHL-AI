#!/usr/bin/env node
/**
 * GHL Voice AI — Advanced Mode System Prompt Generator
 * Usage: node system-prompt-generator.js [path/to/intake-form.json]
 * Output: system-prompt-output.txt (copy-paste into GHL > AI Agents > Voice AI > Inbound > Advanced Mode)
 */
...
if (!fs.existsSync(intakePath)) {
  console.error(`ERROR: Intake form not found at ${intakePath}`);
  process.exit(1);
}

const b = JSON.parse(fs.readFileSync(intakePath, 'utf8'));

// Validate required fields
const required = ['business_name', 'agent_name', 'industry', 'services', 'technician_transfer_number', 'timezone'];
const missing = required.filter(f => !b[f] || (Array.isArray(b[f]) && b[f].length === 0));
if (missing.length) {
  console.error(`ERROR: Missing required intake fields: ${missing.join(', ')}`);
  process.exit(1);
}

if (!b.technician_transfer_number) {
  console.error('BLOCKER: technician_transfer_number is empty. Human handoff is non-negotiable. Add a transfer number before generating.');
  process.exit(1);
}
...
const formatHours = (hours) => {
  return Object.entries(hours)
    .map(([day, h]) => `  ${day.charAt(0).toUpperCase() + day.slice(1)}: ${h.open === 'closed' ? 'Closed' : `${h.open} – ${h.close}`}`)
    .join('\n');
};
...
const outputPath = path.join(__dirname, 'system-prompt-output.txt');
fs.writeFileSync(outputPath, prompt, 'utf8');

console.log(`\n✅ System prompt generated: ${outputPath}`);
console.log(`   Word count: ${wordCount} / 2000`);
if (wordCount > 2000) {
  console.warn(`\n⚠️  WARNING: Prompt exceeds 2,000 words (${wordCount}). Move excess detail to Knowledge Base content.`);
}
console.log('\nNext step: Copy the contents of system-prompt-output.txt into GHL > AI Agents > Voice AI > Inbound > [Agent] > Advanced Mode > System Prompt\n');
